...
def confirm_email(token):
    ...
    # Auto-auth after confirm or redirect to auth page
    safe_next = _get_safe_next_param('next', user_manager.after_confirm_endpoint)
    if user_manager.auto_login_after_confirm:
        return _do_login_user(user, safe_next)                       # auto-auth
    else:
        return redirect(url_for('user.auth')+'?next='+quote(safe_next))    # redirect to auth page
...
def login():
    ...
    # Initialize form
    login_form = user_manager.login_form(request.form)          # for auth.html
...
def register():
    ...
    if request.method=='POST' and register_form.validate():
        ...
        # Auto-auth after register or redirect to auth page
        ...
        if user_manager.auto_login_after_register:
            return _do_login_user(user, safe_reg_next)                     # auto-auth
        else:
            return redirect(url_for('user.auth')+'?next='+quote(safe_reg_next))  # redirect to auth page
...
def reset_password(token):
    ...
    if request.method=='POST' and form.validate():
        ...
        # Auto-auth after reset password or redirect to auth page
